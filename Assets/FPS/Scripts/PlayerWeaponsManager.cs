...
public class PlayerWeaponsManager : MonoBehaviour
{
    ...
    [Tooltip("Level 1")]
    public List<WeaponController> startingWeaponsLevel1 = new List<WeaponController>();

    [Tooltip("Level 2")]
    public List<WeaponController> startingWeaponsLevel2 = new List<WeaponController>();

    [Tooltip("Level 3")]
    public List<WeaponController> startingWeaponsLevel3 = new List<WeaponController>();
    ...
    // weijie - produce a list of shorthanded weapon names for serialization
    public List<string> getInventoryWeaponNames() {
        List<string> weaponNameList = new List<string>();
        foreach (var wpc in m_WeaponSlots){
            if (wpc != null){
                switch(wpc.weaponName) {
                    case "Blaster":
                        weaponNameList.Add("blaster");
                        break;
                    case "Disc Launcher" :
                        weaponNameList.Add("launcher");
                        break;
                    case "Shotgun" :
                        weaponNameList.Add("shotgun");
                        break;
                    default:
                        continue;
                }
    ...
    private void Start()
    {
        List<List<WeaponController>> all_weapons = new List<List<WeaponController>>();
        all_weapons.Add(startingWeaponsLevel1);
        all_weapons.Add(startingWeaponsLevel2);
        all_weapons.Add(startingWeaponsLevel3);



        pickupFactory = FindObjectOfType<PickupFactory>();

        PlayerInfo loadedData = DataSaver.loadData<PlayerInfo>("playerInfo");
        if (PlayerLevel.level != 1 && loadedData != null){
            foreach (string weaponName in loadedData.weapons){
                WeaponController inventoryWeapon = pickupFactory.getWeaponController(weaponName);
                AddWeapon(inventoryWeapon);
            }
        }
        else{
            // load starting weapons at Level 1
            foreach (var weapon in all_weapons[PlayerLevel.level - 1]){
                AddWeapon(weapon);
            }
        }      
        ...
        //foreach (var weapon in all_weapons[m_GameFlowManager.level - 1])
        //{
        //    AddWeapon(weapon);
        //}
        //SwitchWeapon(true);
        ...
        {
            SwitchToWeaponIndex(loadedData.weapons.Count - 1);
        }
    }
